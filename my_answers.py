...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        print('X = {}, self.weights_input_to_hidden = {}'.format(X, self.weights_input_to_hidden))
        ...
        final_outputs = np.sum(final_inputs) # signals from final output layer
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        print('self.weights_hidden_to_output.shape = {}'.format(self.weights_hidden_to_output.shape))
        print('np.divide(y, self.weights_hidden_to_output) = {}'.format(np.divide(y, self.weights_hidden_to_output)))
        hidden_error = np.subtract(hidden_outputs, np.divide(y, self.weights_hidden_to_output).flatten())

        # TODO: Backpropagated error terms - Replace these values with your calculations.
        output_error_term = np.dot(hidden_outputs.T, np.full((2, ), error))

        print('hidden_outputs = {}'.format(hidden_outputs))
        print('hidden_error = {}'.format(hidden_error))
        hidden_error_term = np.dot(X.T, hidden_error * self.__sigmoid_derivative(hidden_outputs))
